...
from langchain_yugabytedb import YugabyteDBVectorStore, YBEngine
...
class TestVectorStoreSearch:
    ...
    @pytest_asyncio.fixture(scope="class")
    async def vs(self, engine: YBEngine) -> AsyncIterator[YugabyteDBVectorStore]:
        await engine.ainit_vectorstore_table(
            DEFAULT_TABLE, VECTOR_SIZE, store_metadata=False
        )
        vs = await YugabyteDBVectorStore.create(
            engine,
    ...
    @pytest_asyncio.fixture(scope="class")
    async def engine_sync(self) -> AsyncIterator[YBEngine]:
        engine = YBEngine.from_connection_string(url=CONNECTION_STRING)
        yield engine
    ...
    @pytest_asyncio.fixture(scope="class")
    async def vs_custom(self, engine_sync: YBEngine) -> AsyncIterator[YugabyteDBVectorStore]:
        ...
        vs_custom = YugabyteDBVectorStore.create_sync(
            engine_sync,
            embedding_service=embeddings_service,
    ...
    @pytest_asyncio.fixture(scope="class")
    async def vs_custom_filter(self, engine: YBEngine) -> AsyncIterator[YugabyteDBVectorStore]:
        await engine.ainit_vectorstore_table(
            CUSTOM_FILTER_TABLE,
            VECTOR_SIZE,
            metadata_columns=[
        ...
        vs_custom_filter = await YugabyteDBVectorStore.create(
            engine,
            embedding_service=embeddings_service,
    ...
    async def test_asimilarity_search_score(self, vs: YugabyteDBVectorStore) -> None:
        results = await vs.asimilarity_search_with_score("foo")
        assert len(results) == 4
        assert results[0][0] == Document(page_content="foo", id=ids[0])
        assert results[0][1] == 0

    async def test_asimilarity_search_by_vector(self, vs: YugabyteDBVectorStore) -> None:
    ...
    async def test_aget_by_ids(self, vs: YugabyteDBVectorStore) -> None:
        test_ids = [ids[0]]
        results = await vs.aget_by_ids(ids=test_ids)

        assert results[0] == Document(page_content="foo", id=ids[0])

    async def test_aget_by_ids_custom_vs(self, vs_custom: YugabyteDBVectorStore) -> None:
        test_ids = [ids[0]]
    ...
    async def test_vectorstore_with_metadata_filters(
        self,
        vs_custom_filter: YugabyteDBVectorStore,
        test_filter: dict,
    ...
    async def test_asimilarity_hybrid_search(self, vs: YugabyteDBVectorStore) -> None:
...
class TestVectorStoreSearchSync:
    ...
    async def vs_custom(self, engine_sync: YBEngine) -> AsyncIterator[YugabyteDBVectorStore]:
        ...
        vs_custom = await YugabyteDBVectorStore.create(
            engine_sync,
            embedding_service=embeddings_service,
    ...
    async def vs_custom_filter_sync(
        self, engine_sync: YBEngine
    ) -> AsyncIterator[YugabyteDBVectorStore]:
        engine_sync.init_vectorstore_table(
        ...
        vs_custom_filter_sync = await YugabyteDBVectorStore.create(
            engine_sync,
            embedding_service=embeddings_service,
    ...
    def test_similarity_search_score(self, vs_custom: YugabyteDBVectorStore) -> None:
        results = vs_custom.similarity_search_with_score("foo")
        assert len(results) == 4
        assert results[0][0] == Document(page_content="foo", id=ids[0])
        assert results[0][1] == 0

    def test_similarity_search_by_vector(self, vs_custom: YugabyteDBVectorStore) -> None:
        embedding = embeddings_service.embed_query("foo")
    ...
    def test_max_marginal_relevance_search_vector(
        self, vs_custom: YugabyteDBVectorStore
    ) -> None:
        embedding = embeddings_service.embed_query("bar")
    ...
    def test_get_by_ids_custom_vs(self, vs_custom: YugabyteDBVectorStore) -> None:
        test_ids = [ids[0]]
    ...
    def test_sync_vectorstore_with_metadata_filters(
        self,
        vs_custom_filter_sync: YugabyteDBVectorStore,
        test_filter: dict,
        expected_ids: list[str],
    ) -> None:
        """Test end to end construction and search."""
    ...
    def test_metadata_filter_negative_tests(
        self, vs_custom_filter_sync: YugabyteDBVectorStore, test_filter: dict
    ) -> None:
    ...
    def test_similarity_hybrid_search(self, vs_custom: YugabyteDBVectorStore) -> None:
