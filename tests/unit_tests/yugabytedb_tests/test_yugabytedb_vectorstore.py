...
async def afetch(engine: YBEngine, query: str) -> Sequence[RowMapping]:
    async def run(engine: YBEngine, query: str) -> Sequence[RowMapping]:
        async with engine._pool.connect() as conn:
...
class TestVectorStore:
    ...
    async def engine_sync(self) -> AsyncIterator[YBEngine]:
        engine_sync = YBEngine.from_connection_string(url=CONNECTION_STRING)
        yield engine_sync
    ...
    def vs_sync(self, engine_sync: YBEngine) -> Iterator[YugabyteDBVectorStore]:
        engine_sync.init_vectorstore_table(DEFAULT_TABLE_SYNC, VECTOR_SIZE)

        vs = YugabyteDBVectorStore.create_sync(
            engine_sync,
            embedding_service=embeddings_service,
    ...
    @pytest_asyncio.fixture(scope="class")
    async def vs_custom(self, engine: YBEngine) -> AsyncIterator[YugabyteDBVectorStore]:
    ...
    async def test_init_with_constructor(self, engine: YBEngine) -> None:
        with pytest.raises(Exception):
            YugabyteDBVectorStore(  # type: ignore
                engine=engine,
                embedding_service=embeddings_service,
                table_name=CUSTOM_TABLE,
                id_column="myid",
    ...
    async def test_post_init(self, engine: YBEngine) -> None:
        with pytest.raises(ValueError):
            await YugabyteDBVectorStore.create(
                engine=engine,
                embedding_service=embeddings_service,
                table_name=CUSTOM_TABLE,
                id_column="myid",
    ...
    async def test_aadd_texts(self, engine: YBEngine, vs: YugabyteDBVectorStore) -> None:
    ...
    async def test_cross_env_add_texts(
        self, engine: YBEngine, vs: YugabyteDBVectorStore
    ) -> None:
        ids = [str(uuid.uuid4()) for i in range(len(texts))]
    ...
    async def test_aadd_docs(self, engine: YBEngine, vs: YugabyteDBVectorStore) -> None:
        ids = [str(uuid.uuid4()) for i in range(len(texts))]
    ...
    async def test_aadd_embeddings(
        self, engine: YBEngine, vs_custom: YugabyteDBVectorStore
    ) -> None:
    ...
    async def test_adelete(self, engine: YBEngine, vs: YugabyteDBVectorStore) -> None:
        ids = [str(uuid.uuid4()) for i in range(len(texts))]
        await vs.aadd_texts(texts, ids=ids)
        results = await afetch(engine, f'SELECT * FROM "{DEFAULT_TABLE}"')
        assert len(results) == 3
    ...
    async def test_add_docs(
        self, engine_sync: YBEngine, vs_sync: YugabyteDBVectorStore
    ) -> None:
        ids = [str(uuid.uuid4()) for i in range(len(texts))]
    ...
    async def test_add_embeddings(
        self, engine_sync: YBEngine, vs_custom: YugabyteDBVectorStore
    ) -> None:
        ...
        vs_custom.add_embeddings(
            ...
            metadatas=[
                {"page": str(i), "source": "yugabytedb"} for i in range(len(texts))
            ],
        )
        results = await afetch(engine_sync, f'SELECT * FROM "{CUSTOM_TABLE}"')
        assert len(results) == 3
    ...
    async def test_from_engine(self) -> None:
        ...
        engine = YBEngine.from_engine(async_engine)
    ...
    async def test_from_engine_loop_connector(
        ...
        engine = YBEngine.from_engine(pool, loop)
        table_name = "test_table" + str(uuid.uuid4()).replace("-", "_")
        await engine.ainit_vectorstore_table(table_name, VECTOR_SIZE)
        vs = await YugabyteDBVectorStore.create(
            engine,
    ...
    def test_get_table_name(self, vs: YugabyteDBVectorStore) -> None:
