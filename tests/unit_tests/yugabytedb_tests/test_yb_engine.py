...
class TestEngineAsync:
    ...
    async def engine(self) -> AsyncIterator[YBEngine]:
        ...
        engine = YBEngine.from_connection_string(url=CONNECTION_STRING, **kwargs)
    ...
    async def test_init_table(self, engine: YBEngine) -> None:
    ...
    async def test_engine_args(self, engine: YBEngine) -> None:
        assert "Pool size: 3" in engine._pool.pool.status()

    async def test_init_table_custom(self, engine: YBEngine) -> None:
    ...
    async def test_init_table_hybrid_search(self, engine: YBEngine) -> None:
        await engine.ainit_vectorstore_table(
            HYBRID_SEARCH_TABLE,
            VECTOR_SIZE,
            id_column="uuid",
    ...
    async def test_invalid_typed_dict(self, engine: YBEngine) -> None:
    ...
    async def test_init_table_custom_with_typed_dict(self, engine: YBEngine) -> None:
        await engine.ainit_vectorstore_table(
            CUSTOM_TYPEDDICT_TABLE,
            VECTOR_SIZE,
            id_column={"name": "uuid", "data_type": "UUID", "nullable": False},
    ...
    async def test_init_table_with_int_id(self, engine: YBEngine) -> None:
        await engine.ainit_vectorstore_table(
            INT_ID_CUSTOM_TABLE,
            VECTOR_SIZE,
    ...
    async def test_from_connection_string_url_error(
        ...
        with pytest.raises(ValueError):
            YBEngine.from_connection_string(
                "postgresql+pg8000://user:password@host:port/db_name",
            )

    async def test_column(self, engine: YBEngine) -> None:
        with pytest.raises(ValueError):
...
class TestEngineSync:
    ...
    async def test_engine_constructor_key(
        ...
        with pytest.raises(Exception):
            YBEngine(key, engine._pool, None, None)
